...
namespace {
    ...
    Platform::FileHandle g_out{};
    UInt64 HashStr(const char* s) noexcept {
        return Fnv1aString(s);
    }
    void EnsureOpen() noexcept {
        if (!g_out.Get()) {
            auto e = Platform::File::Open(StringView("profiling_native.bin"), Platform::FileOpenMode::Append, Platform::FileShareMode::Read);
    ...
    void FlushChunk() noexcept {
        EnsureOpen();
        if (g_out.Get()) {
            (void)Platform::File::Write(g_out, reinterpret_cast<const Byte*>(g_ring), static_cast<USize>(sizeof(g_ring)));
        }
    }
    void FlushPartial() noexcept {
        ...
        if (g_out.Get()) {
            ...
            if (n) {
                (void)Platform::File::Write(g_out, reinterpret_cast<const Byte*>(g_ring), static_cast<USize>(static_cast<USize>(n) * static_cast<USize>(sizeof(Event))));
            }
        }
    }
    void Push(const Event& e) noexcept { g_ring[g_head++ % 1024] = e; if ((g_head % 1024) == 0) FlushChunk(); }
    UInt64 Now() noexcept { return static_cast<UInt64>(Platform::Time::Now()); }
