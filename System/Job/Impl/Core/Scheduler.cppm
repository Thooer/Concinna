...
namespace Sys {
    ...
    Cap::Fiber* Scheduler::CurrentFiber() noexcept { return gCurrentFiber; }
    void Scheduler::SetCurrentFiber(Cap::Fiber* f) noexcept { gCurrentFiber = f; }
...
void WorkerStart(void* p) noexcept {
    ...
    Status Scheduler::Submit(Cap::Job j) noexcept {
    ...
    void Scheduler::ResumeFiber(Cap::Fiber* fb) noexcept {
        if (!fb) return;
        auto* w = static_cast<Worker*>(fb->owner);
        if (!w) return;
        if (fb->prio == static_cast<UInt8>(Cap::QoS::High)) { (void)w->qHigh.PushBottom(fb); }
    ...
    Status Scheduler::SubmitBatch(Cap::Job* jobs, USize count) noexcept {
    ...
    UInt32 Scheduler::NextTimeoutMs() noexcept { return gTimerApi->NextTimeoutMs(); }

    static void ResumeFiberCallback(void* p) noexcept {
        auto* f = static_cast<Cap::Fiber*>(p);
        Scheduler::Instance().ResumeFiber(f);
    }

    Status RunWithCounter(Cap::Job j, Cap::Counter& c) noexcept {
        auto& sch = Scheduler::Instance();
        struct FnPack { void(*fn)(void*) noexcept; void* arg; Cap::Counter* cc; };
        auto real = j.invoke;
        auto realArg = j.arg;
    ...
    Status RunWithCounterPriority(Cap::Job j, Cap::Counter& c, Cap::QoS qos) noexcept {
        j.qos = static_cast<UInt8>(qos);
        return RunWithCounter(j, c);
    }

    Status SubmitPriority(Cap::Job j, Cap::QoS qos) noexcept {
        j.qos = static_cast<UInt8>(qos);
        return Scheduler::Instance().Submit(j);
    }

    void ResumeWaiters(Cap::Counter& c) noexcept {
    ...
    void Suspend(void(*reg)(Cap::Fiber*, void*), void* ctx) noexcept {
        auto* cf = Scheduler::CurrentFiber();
        if (!cf) return;
        cf->state = Cap::FiberState::Waiting;
        if (reg) reg(cf, ctx);
