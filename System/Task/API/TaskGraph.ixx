...
export namespace Sys {
    ...
    struct TaskNode { TaskFunc fn{nullptr}; void* arg{nullptr}; Language::UInt8 qos{0}; };
    struct Edge { Language::USize from; Language::USize to; };

    export struct TaskGraph {
        TaskNode* m_nodes{nullptr}; Edge* m_edges{nullptr}; Language::USize m_nodeCap{0}; Language::USize m_edgeCap{0}; Language::USize m_nodeCount{0}; Language::USize m_edgeCount{0};
        TaskGraph() noexcept {}
        ~TaskGraph() noexcept {
        ...
        bool Reserve(Language::USize nodes, Language::USize edges) noexcept {
            auto h = Prm::Heap::GetProcessDefault();
        ...
        [[nodiscard]] TaskHandle Add(TaskFunc fn, void* arg, Language::UInt8 qos=0) noexcept {
        ...
        [[nodiscard]] bool Dispatch() noexcept {
            ...
            Language::USize indegCap = m_nodeCount;
            auto h = Prm::Heap::GetProcessDefault();
            auto r = Prm::Heap::AllocRaw(h, sizeof(Language::USize) * indegCap);
            if (!r.IsOk()) return false; auto* indeg = static_cast<Language::USize*>(r.Value());
            for (Language::USize i=0;i<m_nodeCount;++i) indeg[i] = 0;
            for (Language::USize i=0;i<m_edgeCount;++i) { auto e = m_edges[i]; ++indeg[e.to]; }
            // 简单的 Kahn 拓扑排序调度
            for (Language::USize round=0; round<m_nodeCount; ++round) {
                bool progressed=false;
                for (Language::USize i=0;i<m_nodeCount;++i) {
                    if (indeg[i]==0 && m_nodes[i].fn) {
                        ...
                        indeg[i] = static_cast<Language::USize>(-1);
                        for (Language::USize k=0;k<m_edgeCount;++k) { auto e = m_edges[k]; if (e.from==i) { if (indeg[e.to] > 0) --indeg[e.to]; } }
                        progressed=true;
                    }
                }
                if (!progressed) break;
