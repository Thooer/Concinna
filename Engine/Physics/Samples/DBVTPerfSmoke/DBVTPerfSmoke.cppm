...
namespace Nova::Samples::PhysicsDBVTPerf {
  export bool Run() noexcept {
    ...
    for (UInt32 i = 0; i < N; ++i) {
      UInt32 gx = i % 100;
      UInt32 gy = (i / 100) % 100;
      Vec3 p{ static_cast<Float32>(gx), static_cast<Float32>(gy), 0.0f };
    ...
    for (UInt32 k = 0; k < 100; ++k) {
      AABB q{ { static_cast<Float32>(k), static_cast<Float32>(k), 0.0f }, { static_cast<Float32>(k + 50), static_cast<Float32>(k + 50), 1.0f } };
      ...
      RayQuery rq{}; rq.origin = { -10.0f, static_cast<Float32>(k + 0.25f), 0.5f }; rq.dir = { 200.0f, 0.0f, 0.0f };
