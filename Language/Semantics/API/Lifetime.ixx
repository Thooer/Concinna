...
export using MaxAlignT = std::max_align_t;
export using AlignValT = std::align_val_t;
...
inline auto MakeDefer(F&& f) noexcept(noexcept(F(static_cast<F&&>(f)))) -> Defer<F> {
        ...
        // 要求删除器在调用上为 noexcept（复用 Lang 概念）
export template<typename Deleter, typename Credential>
...
export template<typename Credential, typename Deleter>
...
export inline void* MemMove(void* dst, const void* src, USize n) noexcept {
