...
pub fn sha256_digest(data: &[u8]) -> [u8; 32] {
    ...
    for chunk in msg.chunks_exact(64) {
        ...
        for i in 16..64 { w[i] = ssig1(w[i-2]).wrapping_add(w[i-7]).wrapping_add(ssig0(w[i-15])).wrapping_add(w[i-16]); }
        let mut a=h[0]; let mut b=h[1]; let mut c=h[2]; let mut d=h[3]; let mut e=h[4]; let mut f=h[5]; let mut g=h[6]; let mut hh=h[7];
        for i in 0..64 {
            let t1 = hh.wrapping_add(bsig1(e)).wrapping_add(ch(e,f,g)).wrapping_add(K[i]).wrapping_add(w[i]);
    ...
    for (i, v) in h.iter().enumerate() {
        let j = i * 4; out[j] = ((v >> 24) & 0xFF) as u8; out[j+1] = ((v >> 16) & 0xFF) as u8; out[j+2] = ((v >> 8) & 0xFF) as u8; out[j+3] = (v & 0xFF) as u8;
    }
    out
}
