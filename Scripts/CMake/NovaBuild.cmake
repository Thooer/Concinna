...
function(add_module_from_dirs target)
  cmake_parse_arguments(ARGS "" "" "INTERFACE_DIR;BACKEND_DIRS;EXCLUDE_FILES" ${ARGN})
  if(NOT ARGS_INTERFACE_DIR)
    message(FATAL_ERROR "add_module_from_dirs(${target}): INTERFACE_DIR is required")
  endif()
  add_library(${target} STATIC)
  # Interface modules
  Collect(_iface_files ${ARGS_INTERFACE_DIR})
  if(_iface_files)
    if(ARGS_EXCLUDE_FILES)
      list(REMOVE_ITEM _iface_files ${ARGS_EXCLUDE_FILES})
    endif()
    target_sources(${target}
      PUBLIC
        FILE_SET CXX_MODULES TYPE CXX_MODULES
          BASE_DIRS ${ARGS_INTERFACE_DIR}
        FILES ${_iface_files}
    )
  endif()
  # Backend modules
  foreach(_bd ${ARGS_BACKEND_DIRS})
    if(EXISTS "${_bd}")
      Collect(_bd_mod_files ${_bd})
      file(GLOB_RECURSE _bd_cpp_files CONFIGURE_DEPENDS "${_bd}/*.cpp")
      if(_bd_mod_files)
        if(ARGS_EXCLUDE_FILES)
          list(REMOVE_ITEM _bd_mod_files ${ARGS_EXCLUDE_FILES})
        endif()
        # Ensure backend module partitions are part of the CXX_MODULES file set for correct BMI export
        target_sources(${target}
          PUBLIC
            FILE_SET CXX_MODULES TYPE CXX_MODULES
              BASE_DIRS ${_bd}
      ...
      if(_bd_cpp_files)
        if(ARGS_EXCLUDE_FILES)
          list(REMOVE_ITEM _bd_cpp_files ${ARGS_EXCLUDE_FILES})
        endif()
        target_sources(${target} PRIVATE ${_bd_cpp_files})
      endif()
    endif()
  endforeach()
...
function(nova_add_module_from_dirs)
  add_module_from_dirs(${ARGV})
endfunction()
...
function(nova_add_module_samples name dir)
  cmake_parse_arguments(ARGS "" "" "LINK" ${ARGN})
  if(ARGS_LINK)
    Sample(${name} ${dir} LINK "${ARGS_LINK}")
  else()
    Sample(${name} ${dir})
  endif()
endfunction()
...
function(API build_name api_dir)
  ...
  # Map last-segment target (e.g., System)
  set_property(GLOBAL PROPERTY "MODULE_TARGET_TO_BUILDNAME_${_target}" "${build_name}")
  # Map prefix+last (e.g., SystemMemory from System.Memory)
  if(_len GREATER 1)
    list(GET _parts 0 _prefix)
    string(CONCAT _prefix_last "${_prefix}" "${_target}")
    set_property(GLOBAL PROPERTY "MODULE_TARGET_TO_BUILDNAME_${_prefix_last}" "${build_name}")
  endif()
...
function(Build target)
  ...
  if(_impl_root)
    file(GLOB _impl_root_files CONFIGURE_DEPENDS "${_impl_root}/*.ixx" "${_impl_root}/*.cppm" "${_impl_root}/*.cpp")
    if(_impl_root_files)
