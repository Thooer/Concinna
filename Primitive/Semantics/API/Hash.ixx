export module Prm.Semantics:Hash;
...
export namespace Prm {
    ...
    constexpr uint64_t Fnv1aBytes(const uint8_t* data, size_t size) noexcept {
    ...
    constexpr uint64_t Fnv1aString(std::string_view str) noexcept {
    ...
    constexpr uint64_t GoldenRatioHash(uint64_t value) noexcept {
        return value * kGoldenRatio64;
    }
    
    // Combine multiple hash values
    constexpr uint64_t CombineHashes(uint64_t seed, uint64_t hash) noexcept {
        return seed ^ (hash + kGoldenRatio64 + (seed << 6) + (seed >> 2));
    }
    
    // Bit mixing for pointer hashing
    constexpr uint64_t BitMixing(uint64_t value) noexcept {
        value ^= value >> 33;
    ...
    constexpr uint64_t SimpleStringHash(std::string_view str) noexcept {
        uint64_t hash = 5381;
        for (char c : str) {
    ...
    constexpr uint64_t tag_invoke(hash_t, std::string_view str) noexcept {
        return Fnv1aString(str);
    }
    
    // Hash for const char*
    constexpr uint64_t tag_invoke(hash_t, const char* str) noexcept {
        return Fnv1aString(str);
    }
