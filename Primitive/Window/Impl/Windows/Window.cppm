...
namespace Prm {
    ...
    static Int64 __stdcall _WndProcBridge(void* hWnd, unsigned int Msg, UInt64 wParam, Int64 lParam) {
        ...
        if (cb) {
            Int64 rc = cb(hWnd, static_cast<UInt32>(Msg), static_cast<UIntPtr>(wParam), static_cast<IntPtr>(lParam));
            if (Msg == WM_NCCREATE && rc == 0) {
    ...
    Status Window::SetTitle(WindowHandle h, StringView title) noexcept {
        if (!h.Get()) return Err(StatusDomain::System(), StatusCode::InvalidArgument);
        const char* ctitle = reinterpret_cast<const char*>(title.data());
        const int ok = SetWindowTextA(reinterpret_cast<HWND>(h.Get()), ctitle);
        return ok ? Ok(StatusDomain::System()) : Err(StatusDomain::System(), StatusCode::Failed);
    }
    ...
    bool Window::ProcessOneMessage(WindowHandle h) noexcept {
        MSG msg{};
        if (PeekMessageA(&msg, reinterpret_cast<HWND>(h.Get()), 0u, 0u, PM_REMOVE)) {
    ...
    // 已移除 Input 状态查询：平台层仅提供 RawInputEvent。
}
